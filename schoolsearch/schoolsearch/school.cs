...
{//Перечислення осіб - 'Teacher', 'Student', 'Undefined' відповідно класу 
    ...
    public enum FilterType { FT_StudentALL, FT_Student, FT_StudentBus, FT_TeacherALL, FT_Teacher, FT_ClassroomStudent, FT_ClassroomTeacher, FT_Bus };
    ...
    public class school
    {
        ...
        public school()
        {
            ...
            //}
            if (!ReadStudentData() || !ReadTeacherData())
                Status = 1;
        }
        ...
        private int GetTeacherIndex(string LastName, string aFirstName, int aClassroom)
        {
            return Teachers.FindIndex(x => x.LastName == LastName && x.FirstName == aFirstName && x.Classroom == aClassroom);
        }
        ...
        public bool ReadStudentData()
        {
            ...
            {
                var lines = File.ReadAllLines(fileName);    // Читаємо файл «list.txt» і зберігаємо його у масиві «lines» 
                for (int i = 0; i < lines.Length; i++)      // Проходимося по всіх рядках у циклі і
                {
                    ...
                    if (items.Length == 5)                  // Перевіряємо, чи знайдено рівно 7 елементів.
                    {
        ...
        public bool ReadTeacherData()
        {
            ...
            if (File.Exists(fileName))                      // Перевіряємо, чи існує файл «students.txt»
            {
                ...
                {
                    string[] items = lines[i].Split(',');   // Розбиваємо рядок на масив рядків з іменами items
                    if (items.Length == 3)                  // Перевіряємо, чи знайдено рівно 7 елементів.
                    {
                        string aTLastName = items[0];           // Якщо знайдено рівно 7 елементів, окремі елементи слід зберегти 
                        string aTFirstName = items[1];          // у змінних aSLastname, aSFirstname, aGrade, aClassroom, aBus,
                        int aClassroom = int.Parse(items[2]);

                        // Потім виконуються методи «AddStudent» та «AddTeacher».
                        AddTeacher(aTLastName, aTFirstName, aClassroom);
                    }
                    else
                    {
                        // Якщо знайдено не рівно 7 елементів, виведіть на консоль помилку «Item mismatch line XXX».
        ...
        public void AddTeacher(string aTLastName, string aTFirstName, int aClassroom)
        {
            try                                                                                         
            {
                ...
                if (GetTeacherIndex(aTLastName, aTFirstName, aClassroom) == -1)
                {
                    Teacher teacher = new Teacher(aTLastName, aTFirstName, aClassroom);         // створюємо новий екземпляр/об'єкт класу «Teacher»
                    ...
                    // Введіть «Teacher» у полі «SearchTeacherClassroom».
                    if (SearchTeacherClassroom.ContainsKey(aClassroom))                                 // Перевіряємо, чи міститься ім'я завантаження в «SearchTeacherClassroom» 
                    {
                        List<int> idxList = SearchTeacherClassroom[aClassroom];                         // «Classroom» вже існує, отримайте збережений Список індексів списку
                        idxList.Add(idxTeachers);                                                       // “Teachers” і додаємо індекс нового “Teacher” до списку
                    }
                    else
                    {
                        List<int> idxList = new List<int>() { idxTeachers };                            // створюємо список для індексів і введіть перший індекс
        ...
        public void GetFilteredData(ref string data, FilterType aFilterType, ref int RecordCount, string aFilterValue = "", int aIntFilterValue = -1)
        {
            ...
            if (aIntFilterValue == -1 && aFilterType == FilterType.FT_ClassroomStudent)
            {
            ...
            switch (aFilterType)
            {
                ...
                case FilterType.FT_ClassroomStudent:
                    {
                        RecordCount = 0;
                        ...
                        {
                            data = "No students for this classroom found!";           // Вивід для консолі «No person for this classroom found!»
                            return;                                                 // Метод виходу
                        }
                       else    
                        {
                    ...
                    break;
                case FilterType.FT_ClassroomTeacher:
                    {
                        RecordCount = 0;
                        // Перевірити, чи значення фільтра, введене для «Прізвище»,
                        // не міститься в MAP «SearchStudentClassroom» і не міститься в MAP «SearchTeacherClassroom»
                        if (!SearchTeacherClassroom.ContainsKey(aIntFilterValue))
                        {
                            data = "No teacher for this classroom found!";           // Вивід для консолі «No person for this classroom found!»
                            return;                                                 // Метод виходу
                        }
                        else
                        {
